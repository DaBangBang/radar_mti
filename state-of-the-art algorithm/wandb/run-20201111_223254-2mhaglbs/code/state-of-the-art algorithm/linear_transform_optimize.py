...
class Affine(nn.Module):
    ...
    def forward(self, t_r, t_z):
        r_out = t_r*self.w1 
        z_out = t_z*self.w2
        return r_out, z_out, self.w1, self.w2
...
def train_function(train_loader):
    ...
    for i, (train_range, train_zeta, train_label) in enumerate(train_loader, 0):
        ...
        optimizer.zero_grad()
        r_out, z_out, w1, w2 = model(train_range, train_zeta)
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        if epoch%1000 == 0:
            ...
            np.save(save_dir + 'expect_r_%4_2dfft_pad_op_scale', np.array(r_out))
            np.save(save_dir + 'expect_z_%4_2dfft_pad_op_scale', np.array(z_out))
            np.save(save_dir + '2dfft_op_param_scale', np.array(var))