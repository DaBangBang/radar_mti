...
learning_rate = 0.001
mm = 1e-3
bin_resolution = 46.8410 / (3/2) ## millimeter = 4.6 cm
train_all = []
test_all = []
train_label_all = []
test_label_all = []
...
def meshgrid():
    m_r = torch.arange(0, bin_resolution*50, bin_resolution).to(device)
    return m_r
...
def data_preparation(data_real, label):
    ...
    data_fft_modulus = data_fft_modulus[:,:,:int(data_fft_modulus.shape[2]/2)]
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(in_features=13*8, out_features=200)
...
if __name__ == '__main__':
    ...
    for f_name in folder_name:
        count += 1
        real_name = f_name + '/range_fft_zero_pad_100_fir*'
    ...
    torch.save(model.state_dict(), os.path.join(wandb.run.dir, 'AE_filter_2.pt'))
