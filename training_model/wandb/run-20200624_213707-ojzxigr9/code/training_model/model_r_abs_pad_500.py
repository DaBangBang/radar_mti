...
learning_rate = 0.001
mm = 1e-3
bin_resolution = 46.8410 / (7/2) ## millimeter = 4.6 cm
padding = 500
train_all = []
test_all = []
train_label_all = []
...
def meshgrid():
    m_r = torch.arange(0, bin_resolution*100, bin_resolution).to(device)
    return m_r
...
def data_preparation(data_real, data_imag, label):
    ...
    data_fft_modulus = data_fft_modulus[:,:,:int(data_fft_modulus.shape[2]/7)]
...
class Model(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(in_features=25*8, out_features=200)
...
if __name__ == '__main__':
    ...
    torch.save(model.state_dict(), os.path.join(wandb.run.dir, 'AE_3.pt'))
