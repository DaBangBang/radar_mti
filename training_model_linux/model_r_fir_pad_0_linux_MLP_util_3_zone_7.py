...
def evaluation(label, expect_r):
    ...
    np.save(test_dir + 'mae_each_fold_zone_7_MLP', np.array(mae_each_fold))
    np.save(test_dir + 'rmse_each_fold_zone_7_MLP', np.array(sd_each_fold))
...
if __name__ == '__main__':
    ...
    if args.save_to_wandb:
        run = wandb.init(project="model_zone_generalization", name="test_range_zone_7_MLP_"+str(fold), dir='/home/nakorn/weight_bias', reinit=True)
    ...
    # np.save(data_save_path, '/select_train_zone_7_MLP', select_train)
    # np.save(data_save_path, '/select_test_zone_7_MLP', select_test)
    # ============================================ 

    print(train_data.shape, train_label.shape, test_data.shape, test_label.shape)

    np.save(test_dir + 'train_index_fold_zone_7_MLP' + str(fold), np.array(train_index))
    np.save(test_dir + 'test_index_fold_zone_7_MLP' + str(fold), np.array(test_index))
    ...
    if args.test_only:
        ...
        np.save(save_predict_path + 'expect_r_zone_7_MLP', expect_r)
        ...
        for epoch in range(args.epochs):
            ...
            if epoch%10 == 0:
                ...
                np.save(test_dir + 'expec_r_fold_zone_7_MLP' + str(fold), np.array(expect_r))
            ...
            if args.save_to_wandb and (epoch%500 == 0):
                torch.save(model.state_dict(), os.path.join(wandb.run.dir, 'model_test_zone_7_MLP'+str(fold)+'_ep_'+str(epoch)+'.pt'))
        run.finish()
