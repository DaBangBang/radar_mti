...
def evaluation(label, expect_r):
    ...
    np.save(test_dir + 'mae_each_10foil_70%', np.array(mae_each_fold))
    np.save(test_dir + 'rmse_each_10foil_70%', np.array(sd_each_fold))
...
if __name__ == '__main__':
    ...
    for train_index, test_index in kf.split(data_iq):
        ...
        if args.save_to_wandb:
            run = wandb.init(project="localization_net_experiment", name="10_foil_range_pad3_70%"+str(fold), dir='/home/nakorn/weight_bias', reinit=True)
        
        reject_list = np.random.choice(np.arange(0,108), size=33 , replace= False) ## random drop 30% of data (70% training)
        ...
        np.save(test_dir + 'train_index_70%_fold_' + str(fold), np.array(train_index))
        np.save(test_dir + 'test_index_70%_fold_' + str(fold), np.array(test_index))
        ...
        if args.test_only:
            ...
            np.save(save_predict_path + 'expect_r_10foil_70%', expect_r)
            ...
            for epoch in range(args.epochs):
                ...
                if epoch%10 == 0:
                    ...
                    np.save(test_dir + 'expec_r_70%_fold_' + str(fold), np.array(expect_r))
                ...
                if args.save_to_wandb and (epoch%500 == 0):
                    torch.save(model.state_dict(), os.path.join(wandb.run.dir, 'model_r_10fold_70%_'+str(fold)+'_ep_'+str(epoch)+'.pt'))
            run.finish()
