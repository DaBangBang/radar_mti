...
def evaluation(label, expect_z):
    ...
    np.save(test_dir + 'mae_aoa_each_fold_zone_6', np.array(mae_each_fold))
    np.save(test_dir + 'rmse_aoa_each_fold_zone_6', np.array(sd_each_fold))
    ...
    if args.test_only:
        ...
        for epoch in range(args.epochs):
            ...
            if epoch%10 == 0:
                ...
                np.save(test_dir + 'expect_z_zone_6' + str(fold), np.array(expect_z))
            ...
            if args.save_to_wandb and (epoch%500 == 0):  
                torch.save(model.state_dict(), os.path.join(wandb.run.dir, 'aoa_zone_6_'+str(fold)+'_ep_'+str(epoch)+'.pt'))
        run.finish()
