...
def evaluation(label, expect_z):
    ...
    np.save(test_dir + 'mae_aoa_each_fold_zone_10_reg', np.array(mae_each_fold))
    np.save(test_dir + 'rmse_aoa_each_fold_zone_10_reg', np.array(sd_each_fold))
...
if __name__ == '__main__':
    ...
    if args.save_to_wandb:
        run = wandb.init(project="model_zone_generalization", name="test_aoa_zone_10_reg"+str(fold), dir='/home/nakorn/weight_bias', reinit=True)
    ...
    # np.save(data_save_path+ '/select_aoa_train_zone_10_reg', select_train)
    # np.save(data_save_path+ '/select_aoa_test_zone_10_reg', select_test)
    # ============================================ 

    print(train_data.shape, train_label.shape, test_data.shape, test_label.shape)
    np.save(test_dir + 'train_aoa_index_zone_10_reg' + str(fold), np.array(train_index))
    np.save(test_dir + 'test_aoa_index_zone_10_reg' + str(fold), np.array(test_index))
    ...
    if args.test_only:
        ...
        for epoch in range(args.epochs):
            ...
            if epoch%10 == 0:
                ...
                np.save(test_dir + 'expect_z_aoa_zone_10_reg' + str(fold), np.array(expect_z))
            ...
            if args.save_to_wandb and (epoch%500 == 0):  
                torch.save(model.state_dict(), os.path.join(wandb.run.dir, 'aoa_zone_10_reg_'+str(fold)+'_ep_'+str(epoch)+'.pt'))
        run.finish()
