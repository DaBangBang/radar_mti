...
def evaluation(label, expect_r):
    ...
    np.save(test_dir + 'mae_each_fold_zone_4', np.array(mae_each_fold))
    np.save(test_dir + 'rmse_each_fold_zone_4', np.array(sd_each_fold))
...
if __name__ == '__main__':
    ...
    np.save(test_dir + 'train_index_fold_zone_4' + str(fold), np.array(train_index))
    np.save(test_dir + 'test_index_fold_zone_4' + str(fold), np.array(test_index))
    ...
    if args.test_only:
        ...
        np.save(save_predict_path + 'expect_r_zone_4', expect_r)
        ...
        for epoch in range(args.epochs):
            ...
            if args.save_to_wandb and (epoch%500 == 0):
                torch.save(model.state_dict(), os.path.join(wandb.run.dir, 'model_test_zone_4'+str(fold)+'_ep_'+str(epoch)+'.pt'))
        run.finish()
